...
public class Test {
	 ...
	 * new HyperGraph(start, end);
	 * 
	 * Node e = new Node("E"); Node f = new Node("F"); Node g = new
	 * Node("G"); Node h = new Node("H");
	 * 
	 * hg.nodes.add(e); hg.nodes.add(f); hg.nodes.add(g); hg.nodes.add(h);
	 * 
	 ...
	 * HyperEdge("J", 2, 8.0);
	 * 
	 * hg.hEdges.add(b); hg.hEdges.add(c); hg.hEdges.add(d);
	 * hg.hEdges.add(i); hg.hEdges.add(j);
	 * 
	 * 
	 ...
	 * 
	 * 
	 * try { GraphSaver.toDOT(hg.exactAlgorithm()); } catch (IOException e1)
	...
	try {
	    g = GraphLoader.loadGraph("pinchador.hg");
	} catch ( IOException e) {
	    System.out.println("error");
	}
	
	try {
	    GraphSaver.toDOT(g.exactAlgorithm());
	} catch (IOException e) {
