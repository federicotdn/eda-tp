...
public class GraphSaver {
	...
	for(HyperEdge hEdge: hGraph.hEdges.values()){
	    if (subgraph.hEdges.containsKey(hEdge.name)) {
		    ...
		    writer.write("\"" + hEdge.name + "\"[shape=box, height=0.18, fontsize=12, label=\"" + hEdge.name + " ( " + hEdge.weight + " )   "+ hEdge.distance + "\"];");
		}
	    writer.newLine();
	    
	    
	    for(Node node:hEdge.head){
		if (subgraph.nodes.containsKey(node.name) && subgraph.hEdges.containsKey(hEdge.name)) {
